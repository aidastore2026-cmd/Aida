...
export default function App() {
    ...
    const signaturesBlock = includeSignatures ? `
      <div style="display: flex; justify-content: space-around; margin-top: 25px; padding-top: 15px; border-top: 2px dashed #064e3b; page-break-inside: avoid;">
        <div style="text-align: center; width: 250px; color: #0f172a;">
          <div style="font-weight: bold; margin-bottom: ${STORE_SIGNATURE ? '10px' : '35px'}; font-size: 16px; color: #064e3b;">واژۆی لایەنی یەکەم</div>
          ${signatureImage}
          <div style="border-top: 2px solid #0f172a; padding-top: 8px; font-weight: bold;">${party1Name}</div>
        </div>
        <div style="text-align: center; width: 250px; color: #0f172a;">
          <div style="font-weight: bold; margin-bottom: ${PARTY2_SIGNATURE ? '10px' : '35px'}; font-size: 16px; color: #064e3b;">واژۆی لایەنی دووەم</div>
          ${signature2Image}
          ...
          <div class="receipt-container">
            <div class="header">
              ...
              <div class="header-meta">
                ...
                <p>پسوڵە ژمارە: <span style="color:#10b981; font-weight:bold; font-size:16px;">${receiptNo || '-'}</span></p>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-${loggedAppUser?.role === 'admin' ? '4' : '3'} gap-4 md:gap-6`}>
          {loggedAppUser?.role === 'admin' && (
            <div className="bg-emerald-900 text-white p-5 md:p-6 rounded-xl shadow-lg flex items-center justify-between">
              <div><p className="text-emerald-100 mb-1 font-medium text-sm md:text-base">سندوق (نەختینە)</p><h3 className="text-xl md:text-2xl font-bold">${currentCapital.toFixed(2)}</h3></div>
          ...
          <div className="bg-slate-900 text-white p-5 md:p-6 rounded-xl shadow-lg flex items-center justify-between">
            <div><p className="text-slate-300 mb-1 font-medium text-sm md:text-base">کۆی فرۆشراو</p><h3 className="text-xl md:text-2xl font-bold">${totalSales.toFixed(2)}</h3></div>
          ...
          <div className="bg-black text-white p-5 md:p-6 rounded-xl shadow-lg flex items-center justify-between">
            <div><p className="text-slate-400 mb-1 font-medium text-sm md:text-base">قەرزی بازاڕ (قیست)</p><h3 className="text-xl md:text-2xl font-bold">${totalDebtInMarket.toFixed(2)}</h3></div>
          ...
          <div className="bg-emerald-800 text-white p-5 md:p-6 rounded-xl shadow-lg flex items-center justify-between">
            <div><p className="text-emerald-200 mb-1 font-medium text-sm md:text-base">قەرزی بریکارەکان</p><h3 className="text-xl md:text-2xl font-bold">${totalAgentDebt.toFixed(2)}</h3></div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={editingId || 'new'} onSubmit={handleSaveDefinedItem} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div><label className="block text-sm mb-1 text-slate-600">جۆری کاڵا (نموونە: مۆبایل)</label><input required name="itemType" defaultValue={itemToEdit?.type} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">براند (نموونە: ئەپڵ)</label><input required name="brand" defaultValue={itemToEdit?.brand} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ناوی کاڵا</label><input required name="name" defaultValue={itemToEdit?.name} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" placeholder="iPhone 15 Pro Max..." /></div>
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 rounded-lg font-medium transition-colors">{editingId ? 'نوێکردنەوە' : 'زیادکردن'}</button>
              {editingId && <button type="button" onClick={() => setEditingId(null)} className="bg-slate-900 hover:bg-black text-white p-2.5 rounded-lg">بەتاڵ</button>}
            </div>
          </form>
        </div>
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[500px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">جۆری کاڵا</th><th className="p-3 md:p-4">براند</th><th className="p-3 md:p-4">ناوی کاڵا (مۆدێل)</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {definedItems.map(i => (
                <tr key={i.id} className="border-b border-slate-100 hover:bg-emerald-50/50"><td className="p-3 md:p-4 font-medium">{i.type}</td><td className="p-3 md:p-4">{i.brand}</td><td className="p-3 md:p-4 font-bold text-emerald-900">{i.name}</td><td className="p-3 md:p-4 flex gap-2"><button onClick={() => setEditingId(i.id)} className="text-emerald-900 bg-emerald-100 hover:bg-emerald-200 p-2 rounded-full"><IconEdit /></button><button onClick={() => deleteDefinedItem(i.id)} className="text-rose-600 bg-rose-100 hover:bg-rose-200 p-2 rounded-full"><IconTrash /></button></td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={editingId || 'new_user'} onSubmit={handleSaveAppUser} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div><label className="block text-sm mb-1 text-slate-600">ناوی بەکارهێنەر</label><input required name="username" defaultValue={userToEdit?.username} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 outline-none focus:ring-2 focus:ring-emerald-900" dir="ltr" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">وشەی تێپەڕ</label><input required name="password" defaultValue={userToEdit?.password} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 outline-none focus:ring-2 focus:ring-emerald-900" dir="ltr" /></div>
            <div>
              <label className="block text-sm mb-1 text-slate-600">ڕۆڵ (دەسەڵات)</label>
              <select required name="role" defaultValue={userToEdit?.role || 'user'} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900">
                <option value="user">بەکارهێنەری ئاسایی (User)</option>
                <option value="admin">ئەدمین (Admin)</option>
              </select>
            </div>
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 rounded-lg font-medium transition-colors">{editingId ? 'نوێکردنەوە' : 'دروستکردن'}</button>
              {editingId && <button type="button" onClick={() => setEditingId(null)} className="bg-slate-900 hover:bg-black text-white p-2.5 rounded-lg transition-colors">بەتاڵ</button>}
            </div>
          </form>
        </div>
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[400px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ناوی بەکارهێنەر</th><th className="p-3 md:p-4">ڕۆڵ</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {appUsers.map(u => (
                <tr key={u.id} className="border-b border-slate-100 hover:bg-emerald-50/50"><td className="p-3 md:p-4 font-medium" dir="ltr">{u.username}</td><td className="p-3 md:p-4">{u.role === 'admin' ? 'ئەدمین' : 'ئاسایی'}</td><td className="p-3 md:p-4 flex gap-2"><button onClick={() => setEditingId(u.id)} className="text-emerald-900 bg-emerald-100 hover:bg-emerald-200 p-2 rounded-full"><IconEdit /></button><button onClick={() => deleteAppUser(u.id)} className="text-rose-600 bg-rose-100 hover:bg-rose-200 p-2 rounded-full"><IconTrash /></button></td></tr>
              ))}
              <tr className="border-b border-slate-100 bg-emerald-50/30"><td className="p-3 md:p-4 font-medium text-slate-500" dir="ltr">{DEFAULT_ADMIN_USER}</td><td className="p-3 md:p-4 text-slate-500">سەرەکی (گۆڕانکاری ناکرێت)</td><td className="p-3 md:p-4 text-slate-400 text-sm">بنچینەیی</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={editingId || 'new'} onSubmit={handleSaveCompany} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div><label className="block text-sm mb-1 text-slate-600">ناوی کۆمپانیا</label><input required name="name" defaultValue={compToEdit?.name} className="w-full border border-slate-300 p-2.5 rounded-lg outline-none bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ژمارەی مۆبایل</label><input required name="phone" defaultValue={compToEdit?.phone} className="w-full border border-slate-300 p-2.5 rounded-lg outline-none bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ناونیشان</label><input name="address" defaultValue={compToEdit?.address} className="w-full border border-slate-300 p-2.5 rounded-lg outline-none bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 rounded-lg font-medium transition-colors">{editingId ? 'نوێکردنەوە' : 'زیادکردن'}</button>
              {editingId && <button type="button" onClick={() => setEditingId(null)} className="bg-slate-900 hover:bg-black text-white p-2.5 rounded-lg transition-colors">بەتاڵ</button>}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[600px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ناو</th><th className="p-3 md:p-4">مۆبایل</th><th className="p-3 md:p-4">ناونیشان</th><th className="p-3 md:p-4">قەرزی ماوە</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {companies.map(c => {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={editingId || 'new'} onSubmit={handleSaveAgent} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div><label className="block text-sm mb-1 text-slate-600">ناوی بریکار</label><input required name="name" defaultValue={agentToEdit?.name} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ژمارەی مۆبایل</label><input required name="phone" defaultValue={agentToEdit?.phone} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ناونیشان</label><input name="address" defaultValue={agentToEdit?.address} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">تێبینی</label><input name="notes" defaultValue={agentToEdit?.notes} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 rounded-lg font-medium transition-colors">{editingId ? 'نوێکردنەوە' : 'زیادکردن'}</button>
              {editingId && <button type="button" onClick={() => setEditingId(null)} className="bg-slate-900 text-white p-2.5 rounded-lg hover:bg-black">بەتاڵ</button>}
            </div>
          </form>
        </div>
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[600px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ناو</th><th className="p-3 md:p-4">مۆبایل</th><th className="p-3 md:p-4">ناونیشان</th><th className="p-3 md:p-4">قەرزی لایە</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {agents.map(a => {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={editingId || 'new'} onSubmit={handleSaveOffice} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div><label className="block text-sm mb-1 text-slate-600">ناوی نوسینگە</label><input required name="name" defaultValue={officeToEdit?.name} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ژمارەی مۆبایل</label><input name="phone" defaultValue={officeToEdit?.phone} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-600">ناونیشان</label><input name="address" defaultValue={officeToEdit?.address} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 rounded-lg font-medium transition-colors">{editingId ? 'نوێکردنەوە' : 'زیادکردن'}</button>
              {editingId && <button type="button" onClick={() => setEditingId(null)} className="bg-slate-900 text-white p-2.5 rounded-lg hover:bg-black">بەتاڵ</button>}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[500px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ناو</th><th className="p-3 md:p-4">مۆبایل</th><th className="p-3 md:p-4">ناونیشان</th><th className="p-3 md:p-4">قەرز (ئێمە قەرزارین)</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {offices.map(o => {
                const debt = getOfficeDebt(o.id);
                return (<tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3 md:p-4 font-medium">{o.name}</td><td className="p-3 md:p-4">{o.phone}</td><td className="p-3 md:p-4">{o.address}</td><td className="p-3 md:p-4 font-bold text-orange-600">${debt.toFixed(2)}</td><td className="p-3 md:p-4 flex gap-2"><button onClick={() => setEditingId(o.id)} className="text-emerald-900 bg-emerald-100 p-2 rounded-full hover:bg-emerald-200"><IconEdit /></button><button onClick={() => deleteOffice(o.id)} className="text-rose-600 bg-rose-100 p-2 rounded-full hover:bg-rose-200"><IconTrash /></button></td></tr>)
              })}
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          <form key={`${editingId || 'new'}-${purMode}-${purEntityType}`} onSubmit={handleSavePurchase} className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-7 gap-4 items-end">
            ...
            <div className="lg:col-span-2">
              ...
              <select required name="paymentType" value={purMode} onChange={e=>{setPurMode(e.target.value); if(e.target.value==='debt' && purEntityType==='none') setPurEntityType('company');}} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900">
            ...
            <div className="lg:col-span-2">
              ...
              <select value={purEntityType} onChange={e=>setPurEntityType(e.target.value)} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900">
            ...
            <div className="lg:col-span-3">
              ...
              {purEntityType === 'none' ? (
                 ...
                 <select required={purMode === 'debt'} name="entityId" defaultValue={purToEdit?.companyId} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900">
            ...
            <div className="lg:col-span-7 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 mt-2 space-y-3">
               <h4 className="font-bold text-emerald-900 mb-2 border-b border-emerald-200 pb-2">لیستی کاڵاکان</h4>
               {purItems.map((item, index) => (
                   <div key={item.id} className="flex flex-col sm:flex-row gap-3 sm:items-end p-3 sm:p-0 border sm:border-0 rounded-lg sm:rounded-none bg-white sm:bg-transparent">
                       <div className="flex-1">
                         ...
                         <select required value={item.itemName} onChange={(e) => updatePurItem(item.id, 'itemName', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900">
                           <option value="">هەڵبژێرە...</option>
                       ...
                       <div className="flex gap-3">
                         <div className="w-24 flex-1 sm:flex-none">
                           ...
                           <input required type="number" step="any" value={item.qty} onChange={(e) => updatePurItem(item.id, 'qty', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900" />
                         </div>
                         <div className="w-32 flex-1 sm:flex-none">
                           <label className="block text-xs mb-1 text-slate-600">نرخی تاک</label>
                           <input required type="number" step="any" value={item.unitPrice} onChange={(e) => updatePurItem(item.id, 'unitPrice', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900" />
                         </div>
                       </div>
                       {purItems.length > 1 && (
                          <button type="button" onClick={() => removePurItem(item.id)} className="p-2 w-full sm:w-auto bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-lg transition-colors mt-2 sm:mt-0"><IconTrash/> سڕینەوە</button>
                       )}
                   </div>
               ))}
               <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-4">
                  <button type="button" onClick={addPurItem} className="w-full sm:w-auto text-sm font-bold text-emerald-900 bg-emerald-200 hover:bg-emerald-300 px-4 py-2.5 rounded-lg transition-colors">+ زیادکردنی کاڵای تر</button>
                  <div className="text-left font-black text-xl text-emerald-900 bg-white px-4 py-2 rounded-lg shadow-sm border border-emerald-100">کۆی گشتی: ${purItems.reduce((sum, item) => sum + (Number(item.qty) * Number(item.unitPrice)), 0).toFixed(2)}</div>
               </div>
            </div>

            <div className="lg:col-span-5"><label className="block text-sm mb-1 text-slate-600">تێبینی</label><input name="note" defaultValue={purToEdit?.note} className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" placeholder="هەر تێبینییەک..." /></div>

            <div className="lg:col-span-2 flex gap-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-3 rounded-lg font-bold transition-colors">{editingId ? 'نوێکردنەوە' : 'پاشەکەوتکردنی کڕین'}</button>
              {editingId && <button type="button" onClick={() => {setEditingId(null); setPurItems([{ id: Date.now(), itemName: '', qty: 1, unitPrice: '' }])}} className="bg-slate-900 hover:bg-black text-white p-3 rounded-lg transition-colors">بەتاڵ</button>}
            </div>
          </form>
        </div>
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
          <table className="w-full text-right whitespace-nowrap min-w-[800px]">
            <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ژ.پ</th><th className="p-3 md:p-4">بەروار</th><th className="p-3 md:p-4">فرۆشیار</th><th className="p-3 md:p-4">کاڵاکان</th><th className="p-3 md:p-4">کۆی گشتی</th><th className="p-3 md:p-4">پێدان</th><th className="p-3 md:p-4">تێبینی</th><th className="p-3 md:p-4">کردارەکان</th></tr></thead>
            <tbody>
              {purchases.map(p => {
                ...
                <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-3 md:p-4 font-bold text-emerald-900">${p.total.toFixed(2)}</td><td className="p-3 md:p-4">{p.paymentType === 'debt' ? <span className="bg-slate-200 text-slate-900 px-2 py-1 rounded text-xs font-bold">قەرز</span> : <span className="bg-emerald-100 text-emerald-900 px-2 py-1 rounded text-xs font-bold">کاش</span>}</td><td className="p-3 md:p-4 text-slate-500">{p.note}</td>
                  <td className="p-3 md:p-4 flex gap-2"><button onClick={() => printPurchase(p)} className="text-slate-900 bg-slate-200 hover:bg-slate-300 p-2 rounded-full"><IconPrinter /></button><button onClick={() => handleEditPurchase(p)} className="text-emerald-900 bg-emerald-100 hover:bg-emerald-200 p-2 rounded-full"><IconEdit /></button><button onClick={() => deletePurchase(p.id)} className="text-rose-600 bg-rose-100 hover:bg-rose-200 p-2 rounded-full"><IconTrash /></button></td>
                </tr>
              )})}
            </tbody>
          </table>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
        <table className="w-full text-right min-w-[500px]">
          <thead className="bg-emerald-50 border-b border-slate-200 text-slate-800"><tr><th className="p-3 md:p-4">ناوی کاڵا</th><th className="p-3 md:p-4">تێکڕای نرخی کڕین (تێچوو)</th><th className="p-3 md:p-4">بڕی ماوە لە کۆگا</th></tr></thead>
          <tbody>
            {inventory.length === 0 && <tr><td colSpan="3" className="p-6 text-center text-slate-500">کۆگا بەتاڵە</td></tr>}
            {inventory.map(i => (
              <tr key={i.itemName} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3 md:p-4 font-semibold text-slate-800">{i.itemName}</td><td className="p-3 md:p-4">${i.avgCost.toFixed(2)}</td><td className="p-3 md:p-4 font-bold text-emerald-900 text-lg">{i.currentQty}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      return (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            ...
            <button onClick={() => setViewingInstallments(null)} className="text-white bg-slate-900 px-4 py-2 rounded-lg font-medium self-start sm:self-auto hover:bg-black">گەڕانەوە</button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-emerald-50 p-4 md:p-5 rounded-xl border border-emerald-200"><p className="text-sm text-emerald-800">کۆی بڕی وەرگیراو</p><h3 className="text-xl font-bold text-emerald-900">${totalPaid.toFixed(2)}</h3></div>
            <div className="bg-slate-100 p-4 md:p-5 rounded-xl border border-slate-300"><p className="text-sm text-slate-800">قەرزی ماوە</p><h3 className="text-xl font-bold text-black">${balance.toFixed(2)}</h3></div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 overflow-x-auto">
              ...
              <table className="w-full text-right text-sm min-w-[300px]">
                <thead className="bg-emerald-50 text-emerald-900"><tr><th className="p-2">بەروار</th><th className="p-2">بڕ</th><th className="p-2">تێبینی</th></tr></thead>
                <tbody>
                  {sale.advance > 0 && <tr><td className="p-2">{sale.date}</td><td className="p-2 text-emerald-700 font-bold">${sale.advance.toFixed(2)}</td><td className="p-2">پێشەکی وەرگیراو لەکاتی فرۆشتن</td></tr>}
                  {sale.saleType === 'cash' && <tr><td className="p-2">{sale.date}</td><td className="p-2 text-emerald-700 font-bold">${sale.price.toFixed(2)}</td><td className="p-2">فرۆشتنی کاش بە یەکجار</td></tr>}
                  {paymentsMade.map(p => (<tr key={p.id} className="border-b"><td className="p-2">{p.date}</td><td className="p-2 text-emerald-700 font-bold">${p.amount.toFixed(2)}</td><td className="p-2">{p.note}</td></tr>))}
                </tbody>
              </table>
            </div>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="flex flex-wrap gap-4 mb-6 border-b pb-4">
            <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 bg-slate-50 py-2 px-3 rounded-lg"><input type="radio" name="smode" checked={saleMode === 'installment'} onChange={()=>setSaleMode('installment')} className="w-5 h-5 accent-emerald-900" /> فرۆشتن بە قیست</label>
            <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 bg-slate-50 py-2 px-3 rounded-lg"><input type="radio" name="smode" checked={saleMode === 'cash'} onChange={()=>setSaleMode('cash')} className="w-5 h-5 accent-emerald-900" /> فرۆشتنی کاش</label>
            <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 bg-slate-50 py-2 px-3 rounded-lg"><input type="radio" name="smode" checked={saleMode === 'credit_agent' || saleMode === 'credit'} onChange={()=>setSaleMode('credit_agent')} className="w-5 h-5 accent-emerald-900" /> قەرز بۆ بریکار</label>
            <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 bg-slate-50 py-2 px-3 rounded-lg"><input type="radio" name="smode" checked={saleMode === 'credit_company'} onChange={()=>setSaleMode('credit_company')} className="w-5 h-5 accent-emerald-900" /> قەرز بۆ کۆمپانیا</label>
          </div>
          ...
          <form key={`${editingId || 'new'}-${saleMode}`} onSubmit={handleSaveSale} className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4 items-end">
            ...
            {saleMode === 'credit_agent' || saleMode === 'credit' ? (
              <div className="lg:col-span-2">
                ...
                <select required name="agentId" defaultValue={saleToEdit?.agentId} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900">
                  <option value="">هەڵبژێرە...</option>
              ...
              <div className="lg:col-span-2">
                ...
                <select required name="companyId" defaultValue={saleToEdit?.companyId} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900">
                  <option value="">هەڵبژێرە...</option>
              ...
              <>
                <div className="lg:col-span-2"><label className="block text-sm mb-1 text-slate-600">ناوی کڕیار</label><input required name="customerName" defaultValue={saleToEdit?.customerName} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
                <div><label className="block text-sm mb-1 text-slate-600">ژمارەی مۆبایل</label><input required name="phone" defaultValue={saleToEdit?.phone} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
                <div><label className="block text-sm mb-1 text-slate-600">ناونیشان</label><input required={saleMode==='installment'} name="address" defaultValue={saleToEdit?.address} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
              </>
            )}
            
            <div className="md:col-span-4 lg:col-span-6 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 mt-2 space-y-3">
               <h4 className="font-bold text-slate-700 mb-2 border-b border-emerald-200 pb-2">لیستی کاڵاکان</h4>
               {saleItems.map((item, index) => (
                   <div key={item.id} className="flex flex-col sm:flex-row gap-3 sm:items-end p-3 sm:p-0 border sm:border-0 rounded-lg sm:rounded-none bg-white sm:bg-transparent">
                       <div className="flex-1">
                         <label className="block text-xs mb-1 text-slate-600">ناوی کاڵا</label>
                         <select required value={item.itemName} onChange={(e) => updateSaleItem(item.id, 'itemName', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900">
                       ...
                       <div className="flex gap-3">
                         <div className="w-full sm:w-24">
                           ...
                           <input required type="number" step="any" value={item.qty} onChange={(e) => updateSaleItem(item.id, 'qty', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900" />
                         </div>
                         <div className="w-full sm:w-32">
                           <label className="block text-xs mb-1 text-slate-600">نرخی تاک</label>
                           <input required type="number" step="any" value={item.unitPrice} onChange={(e) => updateSaleItem(item.id, 'unitPrice', e.target.value)} className="w-full border border-slate-300 p-2 rounded-lg focus:ring-2 focus:ring-emerald-900" />
                         </div>
                       </div>
                       {saleItems.length > 1 && (
                          <button type="button" onClick={() => removeSaleItem(item.id)} className="p-2.5 mt-2 sm:mt-0 w-full sm:w-auto bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-lg transition-colors flex justify-center items-center gap-1"><IconTrash/> <span className="sm:hidden">سڕینەوە</span></button>
                       )}
                   </div>
               ))}
               <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-4">
                  <button type="button" onClick={addSaleItem} className="w-full sm:w-auto text-sm font-bold text-emerald-900 bg-emerald-200 hover:bg-emerald-300 px-4 py-2.5 rounded-lg transition-colors">+ زیادکردنی کاڵای تر</button>
                  <div className="text-left font-black text-xl text-emerald-900 bg-white px-4 py-2 rounded-lg border border-emerald-100 shadow-sm w-full sm:w-auto text-center">کۆی گشتی: ${saleItems.reduce((sum, item) => sum + (Number(item.qty) * Number(item.unitPrice)), 0).toFixed(2)}</div>
               </div>
            </div>

            {saleMode === 'installment' && (
              <>
                <div className="lg:col-span-2"><label className="block text-sm mb-1 text-slate-600">پێشەکی وەرگیراو</label><input required type="number" step="any" name="advance" defaultValue={saleToEdit?.advance || 0} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
                <div className="lg:col-span-2"><label className="block text-sm mb-1 text-slate-600">مانگەکانی قیست</label><input required type="number" name="months" min="1" defaultValue={saleToEdit?.months} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" /></div>
              </>
            )}

            {(saleMode === 'credit_agent' || saleMode === 'credit' || saleMode === 'credit_company') && (
              <div className="lg:col-span-2"><label className="block text-sm mb-1 text-slate-600">ماوەی دانەوە (ڕۆژ)</label><input required type="number" name="creditDays" min="1" defaultValue="7" className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900 font-bold text-emerald-900" /></div>
            )}

            <div className="lg:col-span-2"><label className="block text-sm mb-1 text-slate-600">تێبینی</label><input name="note" defaultValue={saleToEdit?.note} className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" placeholder="تێبینی..." /></div>
            
            <div className="md:col-span-6 flex gap-2 mt-2">
              <button type="submit" className="flex-1 bg-emerald-900 hover:bg-emerald-800 text-white p-3 rounded-lg font-bold transition-colors">{editingId ? 'نوێکردنەوە و پاشەکەوتکردن' : 'تۆمارکردنی فرۆشتن'}</button>
              {editingId && <button type="button" onClick={() => {setEditingId(null); setSaleItems([{ id: Date.now(), itemName: '', qty: 1, unitPrice: '' }])}} className="bg-slate-900 text-white p-3 rounded-lg transition-colors hover:bg-black">بەتاڵ</button>}
            </div>
          </form>
        </div>
        <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-slate-200">
            ...
            <tbody>
              {sales.map(s => {
                ...
                <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-3 md:p-4">{itemsList.map(i=>i.itemName).join('، ')}</td>
                  <td className="p-3 md:p-4 font-bold text-emerald-900">${s.price.toFixed(2)}</td><td className="p-3 md:p-4 text-emerald-700">${paid.toFixed(2)}</td><td className="p-3 md:p-4 text-black font-bold">${balance.toFixed(2)}</td>
                  <td className="p-3 md:p-4 flex gap-2">
                    {!isCash && <button onClick={() => setViewingInstallments(s.id)} className="bg-emerald-100 text-emerald-900 px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-emerald-200" title="وردەکاری هەژمار">وردەکاری</button>}
                    {isInst && <button onClick={() => { setViewingDocuments(s); setSelectedDocs((s.documents || []).map(d => typeof d === 'string' ? {name:d, fileUrl:null} : d)); }} className="text-emerald-900 bg-emerald-100 hover:bg-emerald-200 p-2 rounded-full" title="بەڵگەنامەکان"><IconDocs /></button>}
                    {isInst && <button onClick={() => printContract(s)} className="text-slate-900 bg-slate-200 hover:bg-slate-300 p-2 rounded-full" title="چاپکردنی گرێبەست"><IconContract /></button>}
                    <button onClick={() => printSale(s)} className="text-slate-900 bg-slate-200 hover:bg-slate-300 p-2 rounded-full" title="چاپکردنی پسوڵە"><IconPrinter /></button>
                    <button onClick={() => handleEditSale(s)} className="text-emerald-900 bg-emerald-100 hover:bg-emerald-200 p-2 rounded-full" title="دەستکاریکردن"><IconEdit /></button>
                    <button onClick={() => deleteSale(s.id)} className="text-rose-600 bg-rose-100 hover:bg-rose-200 p-2 rounded-full"><IconTrash /></button>
                  </td>
                </tr>
              )})}
            </tbody>
  ...
  const renderPayments = () => {
    ...
    // Search filters for the payment section dropdowns
    const filteredActiveSales = activeSales.filter(s => 
      ...
      <div className="space-y-6">
        ...
        {editingPaymentId && (
          <div className="bg-slate-200 border border-slate-300 p-4 rounded-xl flex justify-between items-center">
            <span className="font-bold text-slate-900">تۆ لە دۆخی دەستکاریکردنی پسوڵەی ژمارە ({pToEdit.receiptNo}) دایت!</span>
            <button onClick={() => setEditingPaymentId(null)} className="text-sm bg-black px-3 py-1 rounded text-white transition-colors hover:bg-slate-800">پاشگەزبوونەوە</button>
          </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {/* وەرگرتنی قیست */}
          <div className={`bg-white p-4 md:p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col ${isEditingReceiveInst ? 'ring-2 ring-emerald-500' : 'border-t-4 border-t-emerald-700'}`}>
            ...
            <form key={`ri-${editingPaymentId || 'new'}`} onSubmit={e => handlePaymentSubmit(e, 'installment')} className="space-y-4 flex-1 flex flex-col">
              ...
              <input required type="number" step="any" name="amount" defaultValue={isEditingReceiveInst ? Math.abs(pToEdit.amount) : ''} placeholder="بڕی وەرگیراو $" className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <input name="note" defaultValue={isEditingReceiveInst ? pToEdit?.note : ''} placeholder="تێبینی..." className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <div className="mt-auto pt-4"><button type="submit" disabled={isEditingCompany || isEditingAgent || isEditingOffice} className="w-full bg-emerald-900 text-white p-2.5 rounded-lg font-bold hover:bg-emerald-800 transition-colors">تۆمارکردن</button></div>
            </form>
          </div>

          {/* مامەڵەی بریکارەکان */}
          <div className={`bg-white p-4 md:p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col ${isEditingAgent ? 'ring-2 ring-emerald-500' : 'border-t-4 border-t-slate-900'}`}>
            ...
            <form key={`ra-${editingPaymentId || 'new'}`} onSubmit={e => handlePaymentSubmit(e, 'agent')} className="space-y-4 flex-1 flex flex-col">
              ...
              <input required type="number" step="any" name="amount" defaultValue={isEditingAgent ? Math.abs(pToEdit.amount) : ''} placeholder="بڕی پارە $" className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              
              {agentTxDir === 'pay' && !isEditingAgent && (
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                   <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 text-sm mb-2"><input type="checkbox" name="viaOffice" checked={viaOfficeAgent} onChange={e=>setViaOfficeAgent(e.target.checked)} className="w-4 h-4 accent-emerald-900" /> پارەدان لەڕێی نوسینگە</label>
                   {viaOfficeAgent && (
              ...
              <input name="note" defaultValue={isEditingAgent ? pToEdit?.note : ''} placeholder="تێبینی..." className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <div className="mt-auto pt-4"><button type="submit" disabled={isEditingCompany || isEditingReceiveInst || isEditingOffice} className="w-full bg-slate-900 text-white p-2.5 rounded-lg font-bold hover:bg-black transition-colors">تۆمارکردن</button></div>
            </form>
          </div>

          {/* مامەڵەی کۆمپانیاکان */}
          <div className={`bg-white p-4 md:p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col ${isEditingCompany ? 'ring-2 ring-emerald-500' : 'border-t-4 border-t-black'}`}>
            ...
            <form key={`pay-${editingPaymentId || 'new'}`} onSubmit={e => handlePaymentSubmit(e, 'company')} className="space-y-4 flex-1 flex flex-col">
              ...
              <input required type="number" step="any" name="amount" defaultValue={isEditingCompany ? Math.abs(pToEdit.amount) : ''} placeholder="بڕی پارە $" className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              ...
              <input name="note" defaultValue={isEditingCompany ? pToEdit?.note : ''} placeholder="ڕەقەم حەواڵە یان تێبینی..." className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <div className="mt-auto pt-4"><button type="submit" disabled={isEditingReceiveInst || isEditingAgent || isEditingOffice} className="w-full bg-black text-white p-2.5 rounded-lg font-bold hover:bg-slate-900 transition-colors">تۆمارکردن</button></div>
            </form>
          </div>

          {/* مامەڵەی نوسینگەکان (حەواڵە) */}
          <div className={`bg-white p-4 md:p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col ${isEditingOffice ? 'ring-2 ring-emerald-500' : 'border-t-4 border-t-emerald-700'}`}>
            ...
            <form key={`off-${editingPaymentId || 'new'}`} onSubmit={e => handlePaymentSubmit(e, 'office')} className="space-y-4 flex-1 flex flex-col">
              ...
              <input required type="number" step="any" name="amount" defaultValue={isEditingOffice ? Math.abs(pToEdit.amount) : ''} placeholder="بڕی پارە $" className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <input name="note" defaultValue={isEditingOffice ? pToEdit?.note : ''} placeholder="تێبینی..." className="w-full border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-emerald-900" />
              <div className="mt-auto pt-4"><button type="submit" disabled={isEditingCompany || isEditingAgent || isEditingReceiveInst} className="w-full bg-emerald-900 text-white p-2.5 rounded-lg font-bold hover:bg-emerald-800 transition-colors">تۆمارکردن</button></div>
            </form>
          </div>
        </div>
              ...
              {recentPayments.map(tx => (
                <tr key={tx.id} className="border-b hover:bg-slate-50">
                  ...
                  <td className="p-3 font-bold">
                    {tx.type === 'receive_installment' && <span className="text-emerald-700">وەرگرتنی قیست</span>}
                    {tx.type === 'receive_agent_payment' && <span className="text-emerald-700">وەرگرتن لە بریکار</span>}
                    {tx.type === 'pay_agent_payment' && <span className="text-black">پێدان بە بریکار</span>}
                    {tx.type === 'pay_company_debt' && <span className="text-black">پێدانی قەرز کۆمپانیا</span>}
                    {tx.type === 'receive_company_payment' && <span className="text-emerald-700">وەرگرتن لە کۆمپانیا</span>}
                    {tx.type === 'receive_office_loan' && <span className="text-emerald-700">وەرگرتن لە نوسینگە</span>}
                    {tx.type === 'pay_office_debt' && <span className="text-black">پێدان بە نوسینگە</span>}
                  </td>
                  <td className="p-3 text-slate-700">{tx.desc} <span className="text-slate-400">({tx.note})</span></td>
                  <td className="p-3 font-bold" dir="ltr"><span className={(tx.type.includes('receive') && tx.type !== 'receive_office_loan') ? 'text-emerald-700' : 'text-slate-900'}>${Math.abs(tx.amount).toFixed(2)}</span></td>
                  <td className="p-3 flex gap-2">
                    <button onClick={() => printPaymentReceipt(tx)} className="bg-slate-200 text-slate-700 hover:bg-slate-300 p-2 rounded-lg text-xs font-semibold"><IconPrinter /></button>
                    {/* Hiding edit for hawala auto-generated txs to prevent bugs. Just delete and recreate if needed */}
                    {(!tx.desc.includes('حەواڵە')) && <button onClick={() => { setEditingPaymentId(tx.id); window.scrollTo({top:0, behavior:'smooth'}); }} className="bg-emerald-100 text-emerald-900 hover:bg-emerald-200 p-2 rounded-lg"><IconEdit /></button>}
                    <button onClick={() => deletePaymentTx(tx.id)} className="bg-slate-200 text-black hover:bg-slate-300 p-2 rounded-lg"><IconTrash /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-emerald-50 p-5 rounded-xl border border-emerald-200">
             <p className="text-sm text-emerald-900 font-bold">سەرمایەی خاوەن کار (دانراو)</p>
             <h3 className="text-2xl font-bold text-emerald-900">${totalInvested.toFixed(2)}</h3>
          </div>
          ...
          <div className="bg-emerald-900 p-5 rounded-xl border border-emerald-800 shadow-md">
             <p className="text-sm text-emerald-100 font-bold">کۆی سەرمایەی بنچینەیی ئێستا</p>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
          ...
          <form onSubmit={handleCapitalSubmit} className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 items-end bg-slate-50 p-5 rounded-xl border border-slate-200">
            <div>
              ...
              <select name="type" className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900">
            ...
            <div><label className="block text-sm mb-1 text-slate-700 font-medium">بڕ (دۆلار)</label><input required type="number" step="any" name="amount" className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
            <div><label className="block text-sm mb-1 text-slate-700 font-medium">هۆکار</label><input required name="reason" className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
            <div className="lg:col-span-2 flex gap-2 items-end">
              <div className="flex-1"><label className="block text-sm mb-1 text-slate-700 font-medium">تێبینی</label><input name="note" className="w-full border border-slate-300 p-2.5 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-emerald-900" /></div>
              <button type="submit" className="bg-emerald-900 hover:bg-emerald-800 text-white p-2.5 px-6 rounded-lg font-bold transition-colors">جێبەجێکردن</button>
            </div>
          </form>
        </div>
              ...
              {[...capitalTx].reverse().map(tx => (
                <tr key={tx.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    ...
                    {tx.type === 'capital_add' && <span className="text-emerald-700 font-medium">سەرمایە (زیاد)</span>}
                    ...
                    {tx.type === 'sale_advance' && <span className="text-emerald-700 font-medium">پێشەکی فرۆشتن</span>}
                    {tx.type === 'sale_cash' && <span className="text-emerald-700 font-medium">فرۆشتنی کاش</span>}
                    {tx.type === 'receive_installment' && <span className="text-emerald-700 font-bold">وەرگرتنی قیست</span>}
                    {tx.type === 'receive_agent_payment' && <span className="text-emerald-700 font-bold">وەرگرتن لە بریکار</span>}
                    ...
                    {tx.type === 'receive_company_payment' && <span className="text-emerald-700 font-bold">وەرگرتن لە کۆمپانیا</span>}
                    {tx.type === 'receive_office_loan' && <span className="text-emerald-700 font-bold">قەرزی نوسینگە (حەواڵە)</span>}
                  ...
                  <td className="p-4 font-bold" dir="ltr"><span className={tx.amount > 0 ? 'text-emerald-700' : 'text-slate-900'}>{tx.amount > 0 ? '+':''}{tx.amount.toFixed(2)}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <button onClick={() => setReportTab('active_accounts')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='active_accounts'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>هەژماری قیستەکان</button>
          <button onClick={() => setReportTab('late')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='late'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>قیستە دواکەوتووەکان</button>
          <button onClick={() => setReportTab('agents_all')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='agents_all'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>بریکارەکان</button>
          <button onClick={() => setReportTab('agents_late')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='agents_late'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>قەرزی دواکەوتووەکان</button>
          <button onClick={() => setReportTab('companies')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='companies'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>کۆمپانیاکان</button>
          <button onClick={() => setReportTab('offices')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='offices'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>نوسینگەکان</button>
          
          {loggedAppUser?.role === 'admin' && (
            <button onClick={() => setReportTab('item_perf')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='item_perf'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>فرۆش و قازانجی کاڵا</button>
          )}
          {loggedAppUser?.role === 'admin' && (
            <button onClick={() => setReportTab('cashbox')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='cashbox'?'bg-emerald-900 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>سندوق</button>
          )}
          <button onClick={() => setReportTab('statement')} className={`px-4 py-2.5 rounded-lg font-semibold transition-colors ${reportTab==='statement'?'bg-blue-600 text-white shadow-md':'bg-white text-slate-700 border hover:bg-emerald-50'}`}>کەشف حساب</button>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-200">
            ...
            <div id="print-cashbox-area" className="overflow-x-auto">
              ...
              <table className="w-full text-right border border-slate-200 text-sm min-w-[700px]">
                <thead className="bg-slate-50"><tr><th className="p-3 border">ژ.پ</th><th className="p-3 border">بەروار</th><th className="p-3 border">جۆر</th><th className="p-3 border">وەسف</th><th className="p-3 border">زیادبوون (داهات)</th><th className="p-3 border">کەمبوون (خەرجی)</th><th className="p-3 border bg-emerald-50">باڵانس</th></tr></thead>
                <tbody>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-200">
            ...
            <div id="tbl-companies" className="overflow-x-auto">
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl mb-4">
                 <h3 className="text-lg font-bold text-emerald-900">کۆی گشتی قەرزی کۆمپانیاکان: <span dir="ltr" className="text-xl md:text-2xl">${totalCompanyDebt.toFixed(2)}</span></h3>
              </div>
              <table className="w-full text-right border border-emerald-200 min-w-[500px]">
                <thead className="bg-emerald-50 text-emerald-900"><tr><th className="p-3 border border-emerald-200">ناوی کۆمپانیا</th><th className="p-3 border border-emerald-200">مۆبایل</th><th className="p-3 border border-emerald-200">ناونیشان</th><th className="p-3 border border-emerald-200">کۆی قەرز</th></tr></thead>
                <tbody>
                  {companiesStatus.map(c => (<tr key={c.id} className="border-b border-emerald-100"><td className="p-3 border border-emerald-200 font-semibold">{c.name}</td><td className="p-3 border border-emerald-200">{c.phone}</td><td className="p-3 border border-emerald-200">${c.address}</td><td className="p-3 border border-emerald-200 font-bold" dir="ltr">{c.debt > 0 ? <span className="text-rose-600">لەسەرمانە: $${c.debt.toFixed(2)}</span> : c.debt < 0 ? <span className="text-blue-600">قەرزارمانن: $${Math.abs(c.debt).toFixed(2)}</span> : <span className="text-slate-500">$0.00</span>}</td></tr>))}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {reportTab === 'offices' && (
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-200">
            ...
            <div id="tbl-offices" className="overflow-x-auto">
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl mb-4">
                 <h3 className="text-lg font-bold text-emerald-900">کۆی گشتی قەرزی نوسینگەکان (لەسەرمان): <span dir="ltr" className="text-xl md:text-2xl">${totalOfficesDebt.toFixed(2)}</span></h3>
              </div>
              <table className="w-full text-right border border-emerald-200 min-w-[500px]">
                <thead className="bg-emerald-50 text-emerald-900"><tr><th className="p-3 border border-emerald-200">ناوی نوسینگە</th><th className="p-3 border border-emerald-200">مۆبایل</th><th className="p-3 border border-emerald-200">ناونیشان</th><th className="p-3 border border-emerald-200">کۆی قەرز لەسەرمان</th></tr></thead>
                <tbody>
                  {officesStatus.map(o => (<tr key={o.id} className="border-b border-emerald-100"><td className="p-3 border border-emerald-200 font-semibold">{o.name}</td><td className="p-3 border border-emerald-200">{o.phone}</td><td className="p-3 border border-emerald-200">${o.address}</td><td className="p-3 border border-emerald-200 text-rose-600 font-bold bg-rose-50/50">${o.debt.toFixed(2)}</td></tr>))}
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-slate-50 p-4 md:p-5 rounded-xl border border-slate-200">
               <div>
                  ...
                  <select value={itemReportFilter.name} onChange={e=>setItemReportFilter({...itemReportFilter, name: e.target.value})} className="w-full border p-2.5 rounded-lg bg-white focus:ring-2 focus:ring-emerald-900 outline-none">
                    <option value="">(سەرجەم کاڵاکان)</option>
                    {definedItems.map(i => <option key={i.id} value={i.name}>{i.brand} - {i.name}</option>)}
                  </select>
               </div>
               <div><label className="block text-sm mb-1 font-semibold text-slate-700">لە بەرواری</label><input type="date" value={itemReportFilter.dateFrom} onChange={e=>setItemReportFilter({...itemReportFilter, dateFrom: e.target.value})} className="w-full border p-2.5 rounded-lg bg-white outline-none focus:ring-2 focus:ring-emerald-900" /></div>
               <div><label className="block text-sm mb-1 font-semibold text-slate-700">بۆ بەرواری</label><input type="date" value={itemReportFilter.dateTo} onChange={e=>setItemReportFilter({...itemReportFilter, dateTo: e.target.value})} className="w-full border p-2.5 rounded-lg bg-white outline-none focus:ring-2 focus:ring-emerald-900" /></div>
            </div>
                ...
                <tbody>
                  ...
                  {itemPerf.rows.map(r => (
                    <tr key={`${r.id}-${r.itemName}`} className="border-b hover:bg-slate-50">
                      ...
                      <td className="p-3 border font-bold text-blue-600">${r.price.toFixed(2)}</td>
  ...
  if (!user) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-900 text-emerald-400 font-bold text-xl" dir="rtl">
  ...
  if (!isLogged) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-900 p-4" dir="rtl" style={{fontFamily: "'Calibri', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif"}}>
        <div className="bg-slate-800 p-6 sm:p-10 rounded-3xl shadow-2xl w-full max-w-md border border-slate-700">
          
          <div className="flex flex-col items-center mb-8">
            {STORE_LOGO && <img src={STORE_LOGO} alt="Logo" className="w-32 h-32 object-contain mb-4 drop-shadow-xl" />}
            <h1 className="text-3xl font-black text-emerald-400 text-center">{STORE_NAME}</h1>
            <p className="text-center text-slate-400 mt-2">چوونەژوورەوەی سیستم</p>
          </div>

          {showForgot ? (
            <div>
              <p className="text-center text-slate-300 mb-6 text-sm">تکایە کۆدی گەڕاندنەوە (Recovery Key) بنووسە</p>
              ...
              {!recoveredData ? (
                <form onSubmit={handleRecover} className="space-y-4">
                  <input 
                    ...
                    className="w-full border border-slate-600 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 text-center bg-slate-900 text-white" 
                    dir="ltr"
                  />
                  {loginError && <p className="text-rose-400 text-sm font-medium text-center">{loginError}</p>}
                  <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-colors">پێشاندانی پاسوۆردەکان</button>
                  <button type="button" onClick={() => {setShowForgot(false); setLoginError('');}} className="w-full text-slate-400 hover:text-white text-sm mt-2 transition-colors">گەڕانەوە</button>
                </form>
              ) : (
                <div className="space-y-4">
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-700 max-h-60 overflow-y-auto space-y-3">
                    {recoveredData.map((ru, i) => (
                      <div key={i} className="bg-slate-800 p-3 rounded shadow-sm border border-slate-600 text-left" dir="ltr">
                        <p className="text-xs text-slate-400 mb-1">Role: {ru.role}</p>
                        <p className="font-bold text-white">User: <span className="text-emerald-400">{ru.username}</span></p>
                        <p className="font-bold text-white">Pass: <span className="text-rose-400">{ru.password}</span></p>
                      </div>
                    ))}
                  </div>
                  <button type="button" onClick={() => {setShowForgot(false); setRecoveredData(null); setRecoveryKey('');}} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-colors">گەڕانەوە بۆ چوونەژوورەوە</button>
                </div>
              )}
            </div>
          ) : (
            <form onSubmit={handleLoginSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">ناوی بەکارهێنەر (User)</label>
                <input 
                  type="text" 
                  autoComplete="username"
                  value={loginForm.user} 
                  onChange={(e) => setLoginForm({...loginForm, user: e.target.value})} 
                  className="w-full border border-slate-600 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-slate-900 text-white transition-all text-left" 
                  dir="ltr"
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">وشەی تێپەڕ (Password)</label>
                <input 
                  type="password" 
              ...
              {loginError && (
                <p className="text-rose-400 bg-rose-950/50 p-3 rounded-lg text-sm font-medium border border-rose-900/50 text-center">
                  {loginError}
                </p>
              )}
              ...
              <button 
                type="submit" 
                className="w-full bg-emerald-600 text-white font-bold text-lg py-3 rounded-xl shadow-lg hover:bg-emerald-500 transition-colors mt-2"
              >
                چوونە ژوورەوە
              </button>
              
              <div className="text-center mt-4">
                <button type="button" onClick={() => setShowForgot(true)} className="text-sm font-medium text-slate-400 hover:text-emerald-400 transition-colors">
                  وشەی تێپەڕم بیرچووە؟
                </button>
              </div>
            </form>
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-50 overflow-hidden" dir={isMobile ? 'ltr' : 'rtl'} style={{fontFamily: "'Calibri', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif"}}>
      ...
      <div className="md:hidden bg-black text-white p-4 flex justify-between items-center shadow-md z-20">
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 bg-emerald-900 hover:bg-emerald-800 rounded-lg transition-colors">
            <IconMenu />
        </button>
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out w-full md:w-64 bg-black text-white flex flex-col shadow-2xl z-50 md:border-l border-emerald-900`} dir="rtl">
        <div className="p-6 text-center border-b border-emerald-900 bg-emerald-900/40 relative">
          <button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden absolute top-4 left-4 text-emerald-300 hover:text-white"><IconX /></button>
          {STORE_LOGO && <img src={STORE_LOGO} alt="Logo" className="w-24 h-24 mx-auto mb-4 object-contain drop-shadow-md" />}
          <h1 className="text-2xl font-black text-emerald-500 tracking-tight">{STORE_NAME}</h1>
        </div>
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto mt-2 text-sm md:text-base">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-semibold ${view === item.id ? 'bg-emerald-800 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-emerald-400'}`}
            >
              <span className={view === item.id ? 'text-white' : 'text-emerald-400'}>{item.icon}</span> <span>{item.label}</span>
            </button>
          ))}
        </nav>
        ...
        <div className="p-4 border-t border-emerald-900 bg-emerald-900/20">
           <div className="flex items-center gap-3 text-emerald-200 mb-3">
              <div className="bg-emerald-800 p-2 rounded-full"><IconUser /></div>
              <div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-md w-full border border-slate-200">
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto p-2">
              {DOC_TYPES.map(dName => {
                ...
                return (
                  <div key={dName} className={`flex items-center justify-between p-3 border rounded-lg transition-colors ${isSelected ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-100 hover:bg-slate-50'}`}>
                    <label className="flex items-center gap-3 cursor-pointer flex-1">
                      <input type="checkbox" checked={isSelected} onChange={() => handleDocToggle(dName)} className="w-5 h-5 accent-emerald-500 rounded" />
                    ...
                    {isSelected && (
                      <div className="flex items-center gap-2">
                        {isUploading ? (
                           <span className="text-xs text-emerald-500 animate-pulse font-bold">... باردەکرێت</span>
                        ) : docItem.fileUrl ? (
                           <span className="text-xs font-bold text-blue-600 bg-blue-100 px-2 py-1 rounded">هاوپێچ کرا</span>
                        ) : (
                           <label className="cursor-pointer text-white bg-emerald-900 hover:bg-emerald-800 w-8 h-8 rounded-full flex items-center justify-center shadow-sm" title="وێنە بگرە یان هەڵبژێرە">
            ...
            <div className="flex justify-between items-center mt-6 gap-3 pt-4 border-t">
              ...
              <div className="flex gap-2">
                 ...
                 <button onClick={handleSaveDocs} className="px-5 py-2.5 bg-emerald-900 hover:bg-emerald-800 text-white rounded-xl font-bold shadow-lg transition-colors">پاشەکەوت</button>
              </div>
            </div>
          </div>
        </div>
